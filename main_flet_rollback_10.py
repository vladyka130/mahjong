...
def main(page: ft.Page):
    ...
    def tile_clicked(tile: Tile):
        ...
        if game_mode == "solitaire2":
            ...
            # Завжди переміщуємо тейл у комірку (не перевіряємо на відповідність з тейлами в комірках)
            ...
            if solitaire2_slots[0] is not None and solitaire2_slots[1] is not None:
                ...
                if solitaire2_slots[0].tile_type == solitaire2_slots[1].tile_type:
                    # Однакові тейли - спочатку даємо час UI оновитися (0.1 секунди), потім чекаємо 0.3 секунди перед видаленням
                    solitaire2_pending_removal = True
                    print(f"DEBUG tile_clicked solitaire2: Знайдено однакові тейли, спочатку чекаю 0.1 секунди для оновлення UI, потім 0.3 секунди перед видаленням")
                    
                    # Спочатку невелика затримка, щоб UI встиг показати тейл у другій комірці
                    def start_removal_timer():
                        nonlocal solitaire2_slots, solitaire2_last_move, solitaire2_pending_removal
                        # Перевіряємо ще раз після затримки
                        if solitaire2_pending_removal and solitaire2_slots[0] is not None and solitaire2_slots[1] is not None:
                            if solitaire2_slots[0].tile_type == solitaire2_slots[1].tile_type:
                                print(f"DEBUG start_removal_timer: Тейли все ще однакові, запускаю затримку 0.3 секунди перед видаленням")
                                ...
                                print(f"DEBUG start_removal_timer: Тейли вже не однакові, не видаляю")
                                solitaire2_pending_removal = False
                        else:
                            print(f"DEBUG start_removal_timer: Прапор не встановлений або одна з комірок порожня, не видаляю")
                            solitaire2_pending_removal = False
                    
                    # Спочатку невелика затримка (0.1 секунди), щоб UI встиг оновитися і показати тейл у другій комірці
                    initial_timer = threading.Timer(0.1, start_removal_timer)
                    initial_timer.start()
